...
class DuplicationTraceable < ApplicationRecord
  ...
  ALL_DUPLICATION_TRACEABLES = [
    DuplicationTraceable::Course,
    DuplicationTraceable::Assessment
  ].freeze

  class << self
    def duplication_traceable_class_of(dependent_object)
      dependent_class_to_duplication_traceable_class_mapping[dependent_object.class.name]
    end
    ...
    def dependent_class_to_duplication_traceable_class_mapping
      ...
      DuplicationTraceable::ALL_DUPLICATION_TRACEABLES.map do |traceable|
        dependent_class = traceable[:name].constantize.dependent_class
        mappings[dependent_class] << traceable[:name] unless dependent_class.nil?
      end
